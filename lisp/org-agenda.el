...
(defvar org-agenda-log-directory "~/Documents/Orglog/"
  "When non-nil, `org-agenda-get-day-entries-extern' will source
  logging info from this directory.")

(defun org-move-log-to-extern ()
  "Move all logbooks from the current file into `org-agenda-log-directory'."
  (interactive)
  (goto-char (point-min))
  (let ((heading-file (file-name-sans-extension
                       (file-name-nondirectory
                        (buffer-file-name))))
        bnd)
    (while (setq bnd (org-next-block-bounds "^:LOGBOOK:$" "^:END:$"))
      (let* ((logbook (buffer-substring-no-properties (car bnd) (cdr bnd)))
             (clock-strs (cl-subseq (split-string logbook "\n" t) 1 -1))
             (heading (nth 4 (org-heading-components))))
        (when (string-match "\\`\\(.*?\\) [0-9]+\\'" heading)
          (setq heading (match-string-no-properties 1 heading)))
        (delete-region (car bnd) (1+ (cdr bnd)))
        (dolist (clock-str clock-strs)
          (org-move-log-entry-to-extern clock-str heading heading-file))))
    (save-some-buffers t)))

(defun org-next-block-bounds (beg-re end-re)
  (let (beg end)
    (when (re-search-forward beg-re nil t)
      (setq beg (match-beginning 0))
      (if (re-search-forward end-re nil t)
          (cons beg (match-end 0))
        (error "Unmatched block %s at %d" beg-re beg)))))

(defun org-move-log-entry-to-extern (clock-str heading heading-file)
  (if (string-match "\\`CLOCK: \\[\\(.*?\\)\\]" clock-str)
      (let* ((start-time (match-string-no-properties 1 clock-str))
             (pt (org-parse-time-string start-time))
             (day (nth 3 pt))
             (month (nth 4 pt))
             (year (nth 5 pt))
             (yname (prin1-to-string year))
             (default-directory org-agenda-log-directory)
             (fname (expand-file-name
                     (format "%d-%02d-%02d.org" year month day) yname)))
        (unless (file-exists-p yname)
          (make-directory yname))
        (with-current-buffer (find-file-noselect fname)
          (goto-char (point-min))
          (unless (re-search-forward (concat "^\\* " heading-file) nil t)
            (goto-char (point-max))
            (unless (bolp)
              (newline))
            (insert "* " heading-file))
  ...
  (when org-agenda-log-directory
    (let ((month (nth 0 date))
      ...
      (let ((fname (expand-file-name
                    (format "%d/%d-%02d-%02d.org" year year month day)
                    org-agenda-log-directory)))
  ...
  (catch 'exit
    ...
    (let* ((span (org-agenda-ndays-to-span (or span org-agenda-span)))
      ...
      (while (setq d (pop day-numbers))
        ...
        (setq rtnall (append rtnall
                             (org-agenda-get-day-entries-extern date)))
